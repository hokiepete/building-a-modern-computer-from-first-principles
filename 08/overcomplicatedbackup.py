...
def write_cmds(commands, filename=None):
    ...
    for cmd in commands:
        ...
        # Generate Label for code block
            ...
            if filename:
                assembly.append(f'({filename}.{FUNC_STK[-1]}${cmd[1]})')
            else:
                assembly.append(f'({FUNC_STK[-1]}${cmd[1]})')
        # GOTO Jumps
        elif cmd[0] == 'if-goto':
            assembly.append(f'{POP}\n@{cmd[1]}\nD;JNE')
        elif cmd[0] == 'goto':
...
if os.path.isdir(srcpath):
    ...
    with open(srcpath + '\\Sys.vm', 'r') as file:
        text = file.read()
    commands = parse_text(text, boot_strap=True)
    assembly = write_cmds(commands, 'Sys')
