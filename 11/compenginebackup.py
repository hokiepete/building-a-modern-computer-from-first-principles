...
class CompilationEngine:
    ...
    def compile_do(self):
        ...
        if varname in self.subroutine_variables:
            ...
            line = f'push {VAR_CONV[varinfo[1]]} {varinfo[2]}\ncall {varinfo[0]}'
            num_exp += 1
            ...
            line = f'push pointer 0\ncall {self.class_name}.{varname}'
            num_exp += 1

        if current_token == "<symbol> ( </symbol>":
            
            _, exp_lst_code = self.compile_expression_list()
    ...
    def compile_term(self):
        ...
        if current_token.startswith('<integerConstant>'):
            ...
            # check for .
                ...
                n, exp_lst_code = self.compile_expression_list()
                line += " " + str(n)
